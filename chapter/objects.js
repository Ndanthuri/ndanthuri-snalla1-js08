 ...
 */
      ...
      function pokerDeck() {
         ...
         function handType(pokerHand) {       
            /* Determine the rank value of each card in the hand by creating a property named rankValue*/
            ...
            function highCard() {
               let maxRank = 0;
               for (let i = 0; i < pokerHand.cards.length; i++) {
                  if (pokerHand.cards[i].rankValue > maxRank) {
                     maxRank = pokerHand.cards[i].rankValue;
                  }
               }
               return maxRank;
            }
            
            /* Function to test for flush in which all five cards have the same suit */
            function hasFlush() {
            ...
            /* Function to test for straight in which the rank value of the cards can be placed in sequential order */
            ...
            /* Function to test for straight flush */
            function hasStraightFlush() {
               return hasFlush() && hasStraight();
            };
      
            /* Function to test for royal flush which consists of 10-J-Q-K-A of the same suit */
            function hasRoyalFlush() {
               return hasStraightFlush() && highCard() === 14;
            };   
      
            /* Function to test for: pairs, two pairs,three of a kind, four of a kind, and full houses */
               function hasSets() {
                  let handSets = {};
                  pokerHand.cards.forEach(function(card) {
                     if (handSets.hasOwnProperty(card.rank)) {
                        handSets[card.rank]++;
                     } else {
                        handSets[card.rank] = 1;
                     }
                  });
